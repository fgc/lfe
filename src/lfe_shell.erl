...
eval_form_1([set|Rest], St0) ->
    {yes,Value,St1} = set(Rest, St0),
    {Value,St1};
eval_form_1([slurp|Args], St0) ->               %Slurp in a file
    {yes,Value,St1} = slurp(Args, St0),
    {Value,St1};
eval_form_1([unslurp|_], #state{base=Be}=St) -> %Forget everything
    {ok,St#state{curr=Be}};
eval_form_1(['define-function',Name,Def], #state{local=Le0,curr=Ce0}=St) ->
    Ar = function_arity(Def),
    Le1 = lfe_eval:add_dynamic_func(Name, Ar, Def, Le0),
    Ce1 = lfe_eval:add_dynamic_func(Name, Ar, Def, Ce0),
    {Name,St#state{local=Le1,curr=Ce1}};
eval_form_1(['define-macro',Name,Def], #state{local=Le0,curr=Ce0}=St) ->
    Le1 = add_mbinding(Name, Def, Le0),
